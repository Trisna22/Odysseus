...
int socks5_auth_userpass(int fd)
{
	...
	if (strcmp(arg_username, username) == 0
		...
		int answer[2] = { AUTH_VERSION, 0xff };
		writen(fd, (void *)answer, ARRAY_SIZE(answer));
		free(username);
		free(password);
		return 1;
...
void *app_thread_process(void *fd)
{
	...
	int version = 0;
	int appSocket = -1;
	char methods = socks_invitation(clientSocket, &version);

	switch (version) {
	case VERSION5: {
			socks5_auth(clientSocket, methods);
			int command = socks5_command(clientSocket);

			if (command == IP) {
				char *ip = socks_ip_read(clientSocket);
				unsigned short int p = socks_read_port(clientSocket);

				appSocket = app_connect(IP, (void *)ip, ntohs(p));
				if (appSocket == -1) {
					app_thread_exit(1, clientSocket);
				}
				socks5_ip_send_response(clientSocket, ip, p);
				free(ip);
				break;
			} else if (command == DOMAIN) {
				unsigned char size;
				char *address = socks5_domain_read(clientSocket, &size);
				unsigned short int p = socks_read_port(clientSocket);

				appSocket = app_connect(DOMAIN, (void *)address, ntohs(p));
				if (appSocket == -1) {
					app_thread_exit(1, clientSocket);
				}
				socks5_domain_send_response(clientSocket, address, size, p);
				free(address);
				break;
			} else {
				app_thread_exit(1, clientSocket);
		...
		// 		unsigned short int p = socks_read_port(clientSocket);
		// 		char *ip = socks_ip_read(clientSocket);
		// 		socks4_read_nstring(clientSocket, ident, sizeof(ident));

		// 		if (socks4_is_4a(ip)) {
		// 			char domain[255];
		// 			socks4_read_nstring(clientSocket, domain, sizeof(domain));
		// 			log_message("Socks4A: ident:%s; domain:%s;", ident, domain);
		// 			appSocket = app_connect(DOMAIN, (void *)domain, ntohs(p));
		// 		} else {
		...
		// 		if (appSocket != -1) {
		// 			socks4_send_response(clientSocket, 0x5a);
		// 		} else {
		// 			socks4_send_response(clientSocket, 0x5b);
		// 			free(ip);
		// 			app_thread_exit(1, clientSocket);
		// 		}
	...
	app_socket_pipe(appSocket, clientSocket);
	close(appSocket);
	app_thread_exit(0, clientSocket);
