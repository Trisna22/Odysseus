...
*/
 ...
 void* processSymbol(char* symbolString) {
     ...
     //if (startsWith(symbolString, PREPENDSYMBOLVALUE "BOFSetOutput")) {
     //    char* internalFunc = NULL;
     //    if (strncmp(symbolString, "__C_specific_handler", 21) == 0) {
 ...
 int parseObject(unsigned char* objData, size_t fileSize) {
     ...
     for (int i = 0; i < coffHeader->numberOfSections; i++) {
         ...
         sectionMapping[i] = (char*)InDirectVirtualAlloc(NULL, coffSection->sizeOfRawData, MEM_COMMIT | MEM_RESERVE | MEM_TOP_DOWN, PAGE_EXECUTE_READWRITE);
     ...
     functionMapping = (char*)InDirectVirtualAlloc(NULL, totalCountRelocations * 8, MEM_COMMIT | MEM_RESERVE | MEM_TOP_DOWN, PAGE_EXECUTE_READWRITE);
